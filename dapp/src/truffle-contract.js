          ...
          var contract = (function (module) {
            ...
            Contract._static_methods = {
              setProvider: function (provider) {
                if (!provider) {
                  throw new Error(
                    'The provided connection information is invalid. A valid provider is  required to connect' +
                      provider
                  )
                }
              ...
              new: function () {
                ...
                if (this.currentProvider == null) {
                  throw new Error(
                    this.contract_name + ' Connect to the network first.'
                  )
                }
                ...
                if (!this.unlinked_binary) {
                  throw new Error(
                    this._json.contract_name +
                      ' Contract code is missing. Compile before deploying.'
                  )
                }
                ...
                return self
                  ...
                  .then(function (network_id) {
                    ...
                    if (unlinked_libraries != null) {
                      ...
                      throw new Error(
                        self.contract_name +
                          ' has unresolved libraries. Deploy and link these before a new version of ' +
            ...
            Contract._properties = {
              ...
              network: function () {
                ...
                if (network_id == null) {
                  throw new Error(
                    this.contract_name +
                      ' lacks network ID, cannot fetch artifact data. Set the network ' +
                      this.contract_name +
                      '.setNetwork(), run ' +
                      this.contract_name +
                ...
                if (this._json.networks[network_id] == null) {
                  throw new Error(
                    this.contract_name +
                      ' needs the  network configuration for its current network id (' +
                      network_id +
                      ').'
                  )
                }
              ...
              address: {
                get: function () {
                  ...
                  if (address == null) {
                    throw new Error(
                      'Unable to access to the deployed address: ' +
                        this.contract_name
                    )
                  }
                ...
                set: function (val) {
                  if (val == null) {
                    throw new Error('Unable to set deployed address: ' + val)
                  }
                  ...
                  if (network_id == null) {
                    throw new Error(
                      this.contract_name +
                        ' No network ID; cannot fetch artifact data. Set the network manually using ' +
                        this.contract_name +
                        '.setNetwork(), run ' +
                        this.contract_name +
                        '.detectNetwork(), or use new(), at(), or deployed() to automatically detect the network.'
                    )
                  }
    ...
    6: [
      function (require, module, exports) {
        ...
        if (!Buffer.TYPED_ARRAY_SUPPORT) {
          console.error(
            'This browser does not support typed arrays (Uint8Array), which are needed for ' +
              '`buffer` v5.x. Please use `buffer` v4.x for compatibility with older browsers.'
          )
        }
          ...
          function getKeys(params, key, allowEmpty) {
            ...
            for (var i = 0; i < params.length; i++) {
              ...
              if (allowEmpty && !value) {
                ...
                throw new Error(
                  '[ethjs-abi] while getKeys found invalid ABI data structure, please text a value is not string'
                )
              }
              result.push(value)
            }
